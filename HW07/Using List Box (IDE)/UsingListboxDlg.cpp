...
BOOL UsingListboxDlg::Cls_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam) 
{
	...
	szClubs.push_back(TEXT("Милан Италия"));
	szClubs.push_back(TEXT("Ливерпуль Англия"));
	szClubs.push_back(TEXT("Бавария Германия"));
	szClubs.push_back(TEXT("Барселона Испания"));
	szClubs.push_back(TEXT("Арсенал Англия"));
	szClubs.push_back(TEXT("Лион Франция"));
	szClubs.push_back(TEXT("Интер Италия"));
	szClubs.push_back(TEXT("Реал Испания"));

	// добавим 8 элементов в список с единичным выбором
	for (size_t i = 0; i < szClubs.size(); i++)
	{
		SendMessage(hList1, LB_ADDSTRING, 0, LPARAM(szClubs[i].c_str()));
	}
...
void UsingListboxDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDC_CHANGENAME:
		{
			int lengthNewStr = SendMessage(hEdit3, WM_GETTEXTLENGTH, 0, 0);
			TCHAR* pBufferNewStr = new TCHAR[lengthNewStr + 1];
			GetWindowText(hEdit3, pBufferNewStr, lengthNewStr + 1);
			...
			if (index != LB_ERR) // выбран ли элемент списка?
			{
				...
				int length = SendMessage(hList1, LB_GETTEXTLEN, index, 0);
				// выделим память необходимого размера
				TCHAR* pBufferOldStr = new TCHAR[length + 1];
				// скопируем название клуба в выделенную память
				SendMessage(hList1, LB_GETTEXT, index, LPARAM(pBufferOldStr));
				// удалим строку из списка
				SendMessage(hList1, LB_DELETESTRING, index, 0);
				SendMessage(hList1, LB_ADDSTRING, 0, LPARAM(pBufferNewStr));
				delete[] pBufferOldStr;
			}
			else
				MessageBox(hwnd, TEXT("Клуб не выбран!"), TEXT("Удаление клуба"), MB_OK | MB_ICONSTOP);
			delete[] pBufferNewStr;
		}
		break;
	case IDC_DELALL:
